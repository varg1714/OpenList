...
- **方案 A（选定）**：`pkg/cron` 内部改为 robfig 实现，旧 API 语义不变（duration 走自定义 `everySchedule` 调度，绕开 `@every` 的亚秒舍入），新增 `NewCronExpr`。
...
```go
func NewCron(d time.Duration) *Cron            // 不变：内部用 everySchedule 固定间隔调度
...
type Cron struct {
    ...
    mu      sync.Mutex
    sched   robfig.Schedule // everySchedule（NewCron 路径）
    expr    string          // 表达式（NewCronExpr 路径；NewCron 路径为空串）
}
```

### 调度器配置

用自定义解析器，同时接受 5 段（Linux 标准）和 6 段（带秒）表达式，并支持描述符；所有 job（`Schedule` 与 `AddFunc` 两条路径）统一套用串行 + 恢复链：
...
s := robfig.New(robfig.WithParser(parser), robfig.WithChain(
    robfig.SkipIfStillRunning(robfig.DefaultLogger),
    robfig.Recover(robfig.DefaultLogger),
))
```

robfig v3.0.1 默认链为空：job 每次触发在新 goroutine 中并发执行，且 panic 会直接杀死进程。因此显式配置链：

- `SkipIfStillRunning`：同一 job 上一次执行未结束时跳过本次触发——恢复旧 ticker 的串行、不重叠语义；
- `Recover`：job panic 时记录日志后继续，不再崩溃。`WithChain` 作用于调度器级别的所有 job，两条注册路径都被包裹。
...
```go
// everySchedule 固定间隔触发，绕开 robfig @every 的 1 秒粒度舍入
...
func NewCron(d time.Duration) *Cron {
    if d <= 0 {
        d = time.Second // 防止零间隔忙循环（否则 Next(t) = t 会死转）
    }
    return &Cron{
        expr:  "", // schedule 路径不走 AddFunc，无需表达式
        s:     s,
...
robfig 的 `@every` 描述符会把亚秒时长向上舍入为 1 秒（`@every 100ms` 实际每 1s 触发一次），因此 duration 路径不委托给描述符，改用自定义 `everySchedule`（`Next = t + d`）保留亚秒精度。行为与旧 ticker 一致：从首次 `Do` 启动调度器起每隔 N 触发，本地时区。
...
func (c *Cron) Do(f func()) {
    if c.sched != nil {
        ...
        if err != nil { panic(err) }  // 表达式已在 NewCronExpr 验证，不可达
        c.entryID = id
    }
...
| 行为 | 旧实现 | 新实现 |
|---|---|---|
| 定时触发 | ticker，创建后每隔 N | everySchedule，启动后每隔 N，等价（`@every` 会亚秒舍入，故不用） |
| 并发执行 | 串行，单 goroutine，未完成的 tick 合并 | SkipIfStillRunning 链：串行不重叠，等价 |
| 触发后 panic | 无恢复，goroutine 退出 | 显式配置 Recover 链，记录日志后继续（robfig v3.0.1 默认链为空，不恢复 panic） |
